...
**Content fragments vs full documents:** If your HTML file starts with `<!DOCTYPE` or `<html`, the server serves it as-is (just injects the helper script). Otherwise, the server automatically wraps your content in the frame template — adding the header, CSS theme, connection status, selection indicator, and all interactive infrastructure. **Write content fragments by default.** Only write full documents when you need complete control over the page.
...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.claude/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.claude/` to `.gitignore` if it's not already there.
...
`SESSION_DIR` is the parent directory of `screen_dir` and `state_dir` (their common `dirname`). If the session used `--project-dir`, mockup files persist in `.claude/brainstorm/` for later reference. Only `/tmp` sessions get deleted on stop.

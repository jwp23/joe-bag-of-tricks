...
- What costs you is reading the same file twice, or in slices — not how many files you took in
  at once. The Edit/Write tool result already confirms a change landed, so a read-back after an
  edit is a second read of the same file. Pulling several files in with one command beats one
  call per file, so batch the ones you already know you need.
...
tests that cover the amended code, and append a fix report to your report file — read the
existing file first, then append below your original report's content, never overwrite it —
covering what you changed, the covering tests you ran, the command, and the output. Reviewers
will not re-run tests for you — your report is the test evidence. Then reply with the same

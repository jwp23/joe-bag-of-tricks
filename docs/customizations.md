...
| skills/implementer-contract | fork-original | — | — | no upstream counterpart; the shared SDD implementer contract, preloaded into the implementer agents (see the paragraph above). 2026-08-14: the "no slash commands or skills" bullet gained its recovery — read the file the procedure lives in and follow it directly, NEEDS_CONTEXT if it cannot be located, never reconstruct from memory. See docs/decisions/plan-steps-name-procedures-not-skills.md |

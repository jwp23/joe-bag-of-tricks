...
| skills/finishing-a-development-branch | replaced | obra/superpowers | MIT | PR-based flow (pr-creator/pr-merger agents; no CI/CodeRabbit here). v6.1.1: ported Step-5 worktree-remove safety (cd to main root first, `prune`, `.worktrees/`-only guard). v6.2.0: ported the base-branch confirmation wording and replaced Common Mistakes + Red Flags with a Common Rationalizations table adapted to the PR flow; upstream's menu/discard/local-merge restructure does not apply (fork has no menu) |
